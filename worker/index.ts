...
async function ensureSchema(env: Env): Promise<void> {
  await env.DB.batch([
    env.DB.prepare(`CREATE TABLE IF NOT EXISTS run_sessions (
      id TEXT PRIMARY KEY,
      seed INTEGER NOT NULL,
      started_at INTEGER NOT NULL,
      consumed_at INTEGER
    ) STRICT`),
    env.DB.prepare(`CREATE TABLE IF NOT EXISTS leaderboard_runs (
      id TEXT PRIMARY KEY,
    ...
    env.DB.prepare(`CREATE INDEX IF NOT EXISTS leaderboard_rank
      ON leaderboard_runs(total_xp DESC, level DESC, duration_seconds ASC, submitted_at ASC)`),
    env.DB.prepare("CREATE INDEX IF NOT EXISTS run_sessions_cleanup ON run_sessions(started_at)"),
  ]);
}
